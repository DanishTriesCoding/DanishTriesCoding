...
class HETActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        with init_context(), warnings.catch_warnings():
            ...
            _load_in_4bit = self.config.model.get("load_in_4bit", False)
            _quant_config = None
            _load_in_4bit = True
            ...
            if _load_in_4bit:
                ...
                print("Loading actor module in 4-bit (QLoRA) via BitsAndBytesConfig")

            actor_module = actor_module_class.from_pretrained(
                pretrained_model_name_or_path=local_path,
                torch_dtype=torch.bfloat16 if _load_in_4bit else torch_dtype,
                quantization_config=_quant_config,
                config=actor_model_config,
                trust_remote_code=trust_remote_code,
            )  # ⭐ Loads the model from the specified path with the given configuration
            print(f"[MEM CHECK] After actor load: {torch.cuda.memory_allocated()/1e9:.2f} GB allocated, {torch.cuda.memory_reserved()/1e9:.2f} GB reserved")
            ...
            if self._is_lora:
                ...
                if _load_in_4bit:
                    actor_module = prepare_model_for_kbit_training(actor_module, use_gradient_checkpointing=False)
        ...
        if fsdp_strategy == "fsdp":
            if _load_in_4bit:
                # Single-GPU QLoRA: FSDP provides no sharding benefit here (NO_SHARD, world_size=1)
                # and forces bf16-sized storage, defeating quantization. Skip wrapping entirely.
